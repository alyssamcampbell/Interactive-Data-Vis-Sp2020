...
function init() {
  ...
  xScale = d3
    .scaleLinear()
    .domain(d3.extent(state.data, d => d.PPR))
  ...
  // AXES
  const xAxis = d3.axisBottom(xScale).tickValues([0,2,4,6,8,10]); // CHANGE THE TICKS ON THE AXIS LIKE THIS -- you can use .tick(5) or the code .tickvalues(INSERT VALUE X,Y,Z that you want here) to define exact values
  const yAxis = d3.axisLeft(yScale).ticks(4); // CHANGE THE TICKS ON THE AXIS HERE
